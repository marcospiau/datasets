...
class DatasetBuilderTestCase(parameterized.TestCase,
  ...
  @classmethod
  @utils.memoize()
  def dummy_data(cls) -> utils.Path:  # pylint: disable=no-self-argument
    ...
    fake_example_dir = utils.Path(test_utils.fake_examples_dir())
    if cls.EXAMPLE_DIR is not None:
      dummy_data_found = utils.Path(cls.EXAMPLE_DIR)
